...
1. [Gurobi.jl](https://github.com/jump-dev/Gurobi.jl): the Julia wrapper of the [Gurobi solver](https://www.gurobi.com/). We used this solver in most of [our tests](XXXXX) and to produce all the figures of [our paper](XXXXXX). We also observed that it is the fastest and most precise of the others we tried, so we suggest running `CALiPPSO` with it (as shown in  the green box above). However, you'll need to install `Gurobi` manually on your computer, as described [before](#Dependencies); keep in mind there are free academic licenses.
