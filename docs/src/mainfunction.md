...
- A `MonoParticle{d,T}` is assigned a position (as an [`SVector`](https://juliaarrays.github.io/StaticArrays.jl/stable/pages/api/#SVector) of size `d` and with [`PeriodicNumber`](@ref) elements of type `T`), a set of neighbours, and the corresponding set of contact vectors and forces. (Note that its size is *not* specified.)
